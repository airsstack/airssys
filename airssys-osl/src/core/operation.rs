...
impl Permission {
    ...
    pub fn operation_type(&self) -> OperationType {
        match self {
            ...
            Permission::ProcessSpawn | Permission::ProcessManage => OperationType::Process,
            Permission::NetworkSocket | Permission::NetworkConnect(_) => OperationType::Network,
    ...
    pub fn grants_access_to(&self, resource: &str) -> bool {
        match self {
            ...
            | Permission::FilesystemExecute(path) => resource.starts_with(path) || path == "*",
            Permission::NetworkConnect(endpoint) => resource == endpoint || endpoint == "*",
            Permission::UtilityExecute(utility) => resource == utility || utility == "*",
...
mod tests {
    ...
    fn test_permission_operation_type_mapping() {
        assert_eq!(
            Permission::FilesystemRead("/tmp".to_string()).operation_type(),
            OperationType::Filesystem
        );
        assert_eq!(
            Permission::ProcessSpawn.operation_type(),
            OperationType::Process
        );
        assert_eq!(
            Permission::NetworkSocket.operation_type(),
            OperationType::Network
        );
        assert_eq!(
            Permission::UtilityExecute("docker".to_string()).operation_type(),
    ...
    fn test_permission_descriptions() {
        ...
        assert_eq!(
            connect_perm.description(),
            "Network connection to 'localhost:8080'"
        );
    }
