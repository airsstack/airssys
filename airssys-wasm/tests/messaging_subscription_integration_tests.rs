...
async fn test_subscription_service_initialization() {
    ...
    assert!(!status.is_running, "Service should not be running initially");
    assert_eq!(status.registered_components, 0);

    // Start service
    let result = service.start().await;
    assert!(result.is_ok(), "Service start should succeed: {:?}", result.err());
    ...
    assert!(subscriber_count >= 1, "Broker should have at least one subscriber after service start");
    ...
    let status = service.status().await;
    assert!(!status.is_running, "Service should not be running after stop");
}
...
async fn test_component_registration_with_subscription_service() {
    ...
    assert!(result.is_ok(), "Component registration should succeed: {:?}", result.err());

    // Verify registered count
    assert_eq!(service.registered_component_count().await, 1, "Should have 1 registered component");
    ...
    broker.publish(MessageEnvelope::new(message)).await.expect("Publish should succeed");

    // Verify message arrives in component's mailbox
    let receive_result = timeout(Duration::from_millis(500), rx.recv()).await;
    match receive_result {
        Ok(Some(ComponentMessage::InterComponent { sender, to, payload })) => {
...
async fn test_full_stack_message_routing() {
    ...
    service.register_component(comp_a.clone(), tx_a).await.unwrap();
    service.register_component(comp_b.clone(), tx_b).await.unwrap();
    service.register_component(comp_c.clone(), tx_c).await.unwrap();
    ...
    broker.publish(MessageEnvelope::new(message_to_b)).await.unwrap();
    ...
    assert!(rx_a.try_recv().is_err(), "Component A should NOT receive message for B");
    assert!(rx_c.try_recv().is_err(), "Component C should NOT receive message for B");
...
async fn test_graceful_shutdown_during_message_flow() {
    ...
    service.register_component(component_id.clone(), tx).await.unwrap();
    ...
    assert!(stop_result.is_ok(), "Graceful shutdown should succeed: {:?}", stop_result.err());
...
async fn test_unregister_during_active_messaging() {
    ...
    broker.publish(MessageEnvelope::new(message1)).await.unwrap();
    ...
    broker.publish(MessageEnvelope::new(message2)).await.unwrap();
    ...
    let status = service.status().await;
    assert!(status.is_running, "Service should still be running after unregister");
...
async fn test_concurrent_registrations() {
    ...
    assert_eq!(service.registered_component_count().await, 20, "Should have 20 registered components");
    ...
    for (component_id, mut rx) in receivers {
        ...
        if let Ok(ComponentMessage::InterComponent { payload, .. }) = received {
            let expected_payload = component_id.as_str().strip_prefix("concurrent-")
                .and_then(|s| s.parse::<u8>().ok())
                .unwrap_or(255);
            assert_eq!(payload, vec![expected_payload], "Component {} got wrong payload", component_id.as_str());
        }
    }
...
async fn test_integration_with_message_delivery_and_reception() {
    ...
    match recv {
        Ok(Some(ComponentMessage::InterComponentWithCorrelation { 
            ...
            assert_eq!(recv_id, correlation_id, "Correlation ID should be preserved");
        }
        Ok(Some(_)) => panic!("Wrong message type received"),
...
async fn test_broker_accessor_returns_same_instance() {
    ...
    assert!(Arc::ptr_eq(&broker, &retrieved_broker), "Broker accessor should return same instance");
}
...
async fn test_status_accuracy_through_lifecycle() {
    ...
    assert_eq!(status.registered_components, 0, "Stage 1: Should have 0 components");
    ...
    for i in 0..3 {
        ...
        service.register_component(ComponentId::new(format!("status-test-{}", i)), tx).await.unwrap();
    }
    let status = service.status().await;
    assert_eq!(status.registered_components, 3, "Stage 3: Should have 3 components");

    // Stage 4: After unregistering one
    service.unregister_component(&ComponentId::new("status-test-1")).await.unwrap();
    let status = service.status().await;
    assert_eq!(status.registered_components, 2, "Stage 4: Should have 2 components after unregister");

    // Stage 5: After stop
    service.stop().await.unwrap();
    let status = service.status().await;
    assert!(!status.is_running, "Stage 5: Should not be running after stop");
