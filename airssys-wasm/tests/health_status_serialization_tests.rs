...
use airssys_wasm::core::multicodec::{Codec, decode_multicodec, encode_multicodec};
...
fn test_health_status_borsh_healthy_round_trip() {
    ...
    // Deserialize
    let decoded = HealthStatus::try_from_slice(&bytes).expect("Borsh deserialization should succeed");
...
fn test_health_status_json_healthy() {
    ...
    // Round-trip
    let decoded: HealthStatus = serde_json::from_str(&json).expect("JSON deserialization should succeed");
...
fn test_health_status_cbor_healthy_round_trip() {
    ...
    let decoded: HealthStatus = serde_cbor::from_slice(&bytes).expect("CBOR deserialization should succeed");
...
fn test_multicodec_borsh_round_trip() {
    ...
    let encoded = encode_multicodec(Codec::Borsh, &borsh_bytes).expect("Multicodec encoding should succeed");
    ...
    let decoded = HealthStatus::try_from_slice(&payload).expect("Borsh deserialization should succeed");
    assert_eq!(decoded, original);
}
...
fn test_multicodec_json_round_trip() {
    ...
    let encoded = encode_multicodec(Codec::JSON, &json_bytes).expect("Multicodec encoding should succeed");
    
    let (codec, payload) = decode_multicodec(&encoded).expect("Multicodec decoding should succeed");
    assert_eq!(codec, Codec::JSON);
    
    let decoded: HealthStatus = serde_json::from_slice(&payload).expect("JSON deserialization should succeed");
    assert_eq!(decoded, original);
}
...
fn test_multicodec_cbor_round_trip() {
    ...
    let encoded = encode_multicodec(Codec::CBOR, &cbor_bytes).expect("Multicodec encoding should succeed");
    
    let (codec, payload) = decode_multicodec(&encoded).expect("Multicodec decoding should succeed");
    assert_eq!(codec, Codec::CBOR);
    
    let decoded: HealthStatus = serde_cbor::from_slice(&payload).expect("CBOR deserialization should succeed");
    assert_eq!(decoded, original);
}
...
fn test_multicodec_all_formats_compatible() {
    ...
    let borsh_bytes = borsh::to_vec(&original).expect("Borsh serialization should succeed");
    let borsh_encoded = encode_multicodec(Codec::Borsh, &borsh_bytes).expect("Multicodec encoding should succeed");
    
    let json_bytes = serde_json::to_vec(&original).expect("JSON serialization should succeed");
    let json_encoded = encode_multicodec(Codec::JSON, &json_bytes).expect("Multicodec encoding should succeed");
    
    let cbor_bytes = serde_cbor::to_vec(&original).expect("CBOR serialization should succeed");
    let cbor_encoded = encode_multicodec(Codec::CBOR, &cbor_bytes).expect("Multicodec encoding should succeed");
    
    // Decode all formats
    let (_, borsh_payload) = decode_multicodec(&borsh_encoded).expect("Multicodec decoding should succeed");
    let borsh_decoded = HealthStatus::try_from_slice(&borsh_payload).expect("Borsh deserialization should succeed");
    
    let (_, json_payload) = decode_multicodec(&json_encoded).expect("Multicodec decoding should succeed");
    let json_decoded: HealthStatus = serde_json::from_slice(&json_payload).expect("JSON deserialization should succeed");
    
    let (_, cbor_payload) = decode_multicodec(&cbor_encoded).expect("Multicodec decoding should succeed");
    let cbor_decoded: HealthStatus = serde_cbor::from_slice(&cbor_payload).expect("CBOR deserialization should succeed");
...
fn test_deserialization_performance() {
    ...
    let start = Instant::now();
    for _ in 0..ITERATIONS {
        let _ = HealthStatus::try_from_slice(&borsh_bytes).expect("Borsh deserialization should succeed");
    }
    let borsh_time = start.elapsed();
    ...
    let start = Instant::now();
    for _ in 0..ITERATIONS {
        let _: HealthStatus = serde_json::from_slice(&json_bytes).expect("JSON deserialization should succeed");
    }
    let json_time = start.elapsed();
    ...
    let start = Instant::now();
    for _ in 0..ITERATIONS {
        let _: HealthStatus = serde_cbor::from_slice(&cbor_bytes).expect("CBOR deserialization should succeed");
    }
...
fn test_health_status_reason_special_characters() {
    ...
    for reason in reasons {
        ...
        let borsh_bytes = borsh::to_vec(&original).expect("Borsh serialization should succeed");
        let borsh_decoded = HealthStatus::try_from_slice(&borsh_bytes).expect("Borsh deserialization should succeed");
        assert_eq!(borsh_decoded, original);
        
        // Test JSON round-trip
        let json = serde_json::to_string(&original).expect("JSON serialization should succeed");
        let json_decoded: HealthStatus = serde_json::from_str(&json).expect("JSON deserialization should succeed");
        assert_eq!(json_decoded, original);
        
        // Test CBOR round-trip
        let cbor_bytes = serde_cbor::to_vec(&original).expect("CBOR serialization should succeed");
        let cbor_decoded: HealthStatus = serde_cbor::from_slice(&cbor_bytes).expect("CBOR deserialization should succeed");
        assert_eq!(cbor_decoded, original);
    }
}
...
fn test_health_status_all_variants() {
    ...
    for variant in variants {
        ...
        let json_decoded: HealthStatus = serde_json::from_slice(&json_bytes).expect("JSON deserialization should succeed");
