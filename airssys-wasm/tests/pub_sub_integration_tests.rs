...
use airssys_wasm::actor::{
    MessageBrokerWrapper, MessagePublisher, SubscriberManager,
};
...
async fn test_multiple_subscribers_same_topic() {
    ...
    manager.subscribe(subscriber_a.clone(), vec!["events".into()]).await.unwrap();
    manager.subscribe(subscriber_b.clone(), vec!["events".into()]).await.unwrap();
    manager.subscribe(subscriber_c.clone(), vec!["events".into()]).await.unwrap();
...
async fn test_correlation_pattern() {
    ...
    let result = requester.publish_with_correlation(
        "requests.data",
        vec![1, 2, 3],
        correlation_id,
    ).await;
...
async fn test_concurrent_publish_subscribe() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            publisher.publish(&format!("topic-{}", i % 3), vec![i as u8]).await
        });
...
async fn test_broadcast_to_multiple_topics() {
    ...
    manager.subscribe(subscriber_a.clone(), vec!["topic-1".into()]).await.unwrap();
    manager.subscribe(subscriber_b.clone(), vec!["topic-2".into()]).await.unwrap();
    manager.subscribe(subscriber_c.clone(), vec!["topic-3".into()]).await.unwrap();
