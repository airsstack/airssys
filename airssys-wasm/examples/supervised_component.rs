...
fn simulate_exponential_backoff(
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        ...
        restart_policy_permanent.should_restart(true)
    );
    println!(
        "  Permanent + normal exit: {}",
        restart_policy_permanent.should_restart(false)
    );
    println!(
        "  Transient + error exit: {}",
        restart_policy_transient.should_restart(true)
    );
    println!(
        "  Transient + normal exit: {}",
        restart_policy_transient.should_restart(false)
    );
    println!(
        "  Temporary + error exit: {}",
        restart_policy_temporary.should_restart(true)
    );
    println!(
        "  Temporary + normal exit: {}",
        restart_policy_temporary.should_restart(false)
    );

    println!(
        "\n✓ Restart limits: {} per {} seconds",
        max_restarts,
        time_window.as_secs()
    );
    ...
    println!(
        "✓ Unhealthy threshold: {} failed checks",
        unhealthy_threshold
    );
    ...
    println!(
        "   - Restart limits: {} per {}s window",
        max_restarts,
        time_window.as_secs()
    );
    println!(
        "   - Health monitoring: {}s interval",
