...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let actor: ComponentActor<MessageStats> = ComponentActor::new(
        component_id.clone(),
        metadata,
        capabilities,
        initial_state,
    ...
    println!("\n--- Final State ---");
    let final_count = actor.with_state(|state| {
        println!("Total messages: {}", state.message_count);
        println!("Error count: {}", state.error_count);
        println!("Avg processing time: {:.2}ms", state.avg_processing_time_ms());
    ...
    let background_task = tokio::spawn(async move {
        ...
        println!("  [Background task] Read message count: {}", guard.message_count);
    });
    ...
    println!("\n=== Example Complete ===");
    println!("Summary: Processed {} messages with {} errors", final_count, 1);
