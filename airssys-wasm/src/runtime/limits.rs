...
/// Memory and resource limits for WASM component execution.
///
/// This struct defines the resource constraints enforced during component execution.
/// All limits are MANDATORY and must be explicitly configured (ADR-WASM-002).
///
/// # Memory Limits
///
/// - **Range**: 512KB (524,288 bytes) to 4MB (4,194,304 bytes)
...
/// "works on my machine" production surprises. The 512KB-4MB range encourages
/// efficient component design while providing enough memory for practical workloads.
///
/// # Examples
///
/// ```rust,ignore
...
impl ResourceLimits {
    /// Minimum memory limit: 512KB (524,288 bytes).
    pub const MIN_MEMORY_BYTES: u64 = 512 * 1024;
    ...
    /// Get the maximum memory bytes allowed.
    ///
    /// # Examples
    ///
    /// ```rust,ignore
...
impl ResourceLimitsBuilder {
    /// Create a new ResourceLimitsBuilder.
    pub fn new() -> Self {
        Self {
    ...
    pub fn build(self) -> WasmResult<ResourceLimits> {
        ...
        if max_memory_bytes < ResourceLimits::MIN_MEMORY_BYTES {
            return Err(WasmError::invalid_configuration(format!(
                "max_memory_bytes ({}) is below minimum ({})",
                max_memory_bytes,
                ResourceLimits::MIN_MEMORY_BYTES
            )));
        }

        if max_memory_bytes > ResourceLimits::MAX_MEMORY_BYTES {
            return Err(WasmError::invalid_configuration(format!(
...
///
/// This struct represents the `[resources.memory]` section in Component.toml
/// and provides conversion to validated ResourceLimits.
///
...
///
/// ```rust,ignore
/// use airssys_wasm::runtime::limits::MemoryConfig;
///
/// let config = MemoryConfig {
///     max_memory_bytes: 2 * 1024 * 1024,
/// };
...
/// # Deserialization
///
/// This struct can be deserialized from TOML configuration files using serde.
/// The deserialized values are validated when converted to ResourceLimits.
///
/// ```rust,ignore
/// use airssys_wasm::runtime::limits::MemoryConfig;
///
...
/// "#;
/// let config: MemoryConfig = toml::from_str(toml_str)?;
/// ```
...
impl MemoryConfig {
    ...
    ///
    /// This method validates that the memory configuration meets the required
    /// constraints (512KB-4MB range) before being used to create ResourceLimits.
    ///
    /// # Errors
    ///
    /// Returns `WasmError::InvalidConfiguration` if:
    ...
    pub fn validate(&self) -> WasmResult<()> {
        if self.max_memory_bytes < ResourceLimits::MIN_MEMORY_BYTES {
            return Err(WasmError::invalid_configuration(format!(
                "max_memory_bytes ({}) is below minimum ({})",
                self.max_memory_bytes,
                ResourceLimits::MIN_MEMORY_BYTES
        ...
        if self.max_memory_bytes > ResourceLimits::MAX_MEMORY_BYTES {
            return Err(WasmError::invalid_configuration(format!(
                "max_memory_bytes ({}) exceeds maximum ({})",
                self.max_memory_bytes,
                ResourceLimits::MAX_MEMORY_BYTES
