...
mod tests {
    ...
    async fn test_service_start_success() {
        ...
        let subscriber_count = broker.subscriber_count().await;
        assert!(subscriber_count >= 1, "Broker should have at least one subscriber");
    ...
    async fn test_service_status_reporting() {
        ...
        service.register_component(ComponentId::new("test"), tx).await.unwrap();
    ...
    async fn test_register_after_unregister() {
        ...
        // Register
        let (tx1, _rx1) = tokio::sync::mpsc::unbounded_channel();
        service.register_component(component_id.clone(), tx1).await.unwrap();
